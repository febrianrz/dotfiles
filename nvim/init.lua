  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      require('mini.ai').setup { 
        n_lines = 500,
        custom_textobjects = {
          -- Add quote text objects explicitly
          ['"'] = { '%b""' },
          ["'"] = { "%b''" },
          ['`'] = { '%b``' },
          -- Extend functionality for other common text objects
...
-- Tutup semua buffer kecuali yang aktif
local function close_other_buffers()
  local current = vim.fn.bufnr()
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    if vim.api.nvim_buf_is_loaded(buf) and vim.api.nvim_buf_get_option(buf, 'buflisted') and buf ~= current then
      vim.api.nvim_buf_delete(buf, {})
    end
  end
end

-- Tutup buffer di sebelah kiri
...
-- Menambahkan keybinding untuk menjalankan fungsi open_git_repo
